	    ...
	    if (this.readyState == 4 && this.status == 200) {
	     	var formattedTime = moment(post.createdAt).format('MMM Do, YYYY h:mm a');
					...
					var html = Mustache.render(template,{
					 	...
					 	'likes' : post.likedBy.length+" likes",
					 	'shares' : post.sharedBy.length+" shares"
					 });
...
$('#shared-posts').click(function(){
	console.log('clicked');
	window.location.href="/sharedpost.html";
});
$('#create-posts').click(function(){
	console.log('clicked');
